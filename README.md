...
編集指示の補助には、追加学習済みの視覚言語モデル [`Qwen/Qwen-Image-2.1-PE-I2I`](https://huggingface.co/Qwen/Qwen-Image-2.1-PE-I2I) を使用します。ボタンを初めて押したときに約 18.8 GB のモデルを追加ダウンロードし、以降はローカルキャッシュを使います。画像と指示の処理はローカルで行います。補助モデルの出力によっては文章が英語になる場合があります。画像の幅・高さは画面の設定を使い、補助モデルが提案するアスペクト比では変更しません。
...
編集指示を整えるときは、画像生成モデルを解放してから補助モデルを読み込みます。補助モデルは処理後に解放するため、その後の画像生成では画像生成モデルを再読み込みします。指示の補助・画像生成・モデル解放は同時に実行しません。
